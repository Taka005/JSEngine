class Engine{
   ...
   */
  constructor(canvas,{fps = 60, gravity = 100, friction = 0.01} = {}){
    this.canvas = canvas;
  ...
  solveSpeed(entity){
    if(entity.speedX > 0){
      const dv = -entity.speedX*Math.min(1,this.friction*entity.size*(1/this.fps));
      entity.speedX += dv;
    }

    if(entity.speedY > 0){
      const dv = -entity.speedY*Math.min(1,this.friction*entity.size*(1/this.fps));
      entity.speedY += dv;
    }
  }
