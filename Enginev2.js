class Engine extends EventTarget {
  ...
  update(){
    ...
    Object.values(this.entities).forEach(entity=>{
      ...
      if(Object.values(entity.contact).length > 0){
  ...
  solvePosition(source,target){
    ...
    if(distance <= source.size + target.size){
      ...
      source.contact[target.name] = true;
      target.contact[source.name] = true;
    }else{
      source.contact[target.name] = false;
      target.contact[source.name] = false;
    }
  }
  ...
  solveGroundPosition(entity,ground){
    ...
    if(distance <= entity.size + ground.size/2){
      ...
      target.contact[ground.name] = true;
    }else{
      target.contact[ground.name] = false;
    }
  }
